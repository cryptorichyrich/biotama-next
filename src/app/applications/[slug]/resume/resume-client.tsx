...
export function TailoredResume({ slug }: { slug: string }) {
  ...
  return (
    <>
      <style>{`
        ...
        @media print { .resume-content { box-shadow: none; } }
      ...
      <div className="resume-content bg-white text-slate-800 p-8 md:p-10 lg:p-12">
        ...
        <header className="border-b border-slate-200 pb-5 mb-5">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1.5 text-sm text-slate-600">
            {socialLinks.map((s) => (
              <a key={s.platform} href={s.url} target="_blank" rel="noopener noreferrer" className="text-slate-600 hover:text-slate-900 underline underline-offset-2 decoration-slate-300">{s.platform}</a>
            ))}
          </div>
        </header>
        ...
        <section className="mb-6">
          ...
          <p className="text-sm text-slate-700 leading-relaxed">
            {app ? app.tailoring.customSummary : profile.summary}
          </p>
        </section>
          ...
          <div className="space-y-5">
            {experiences.map((exp) => (
              <div key={exp.id}>
                ...
                {exp.tech.length > 0 && (
                  <p className="text-xs text-slate-500 mt-1.5"><span className="font-medium">Technologies:</span> {exp.tech.join(", ")}</p>
                )}
              </div>
            ))}
          </div>
        ...
        {/* Key Achievements (from tailoring) */}
          ...
          <section className="mb-6">
            ...
            <ul className="space-y-1">
              {app.tailoring.keyAchievements.map((a: string, i: number) => (
                <li key={i} className="text-xs text-slate-700 leading-relaxed pl-3 relative">
                  <span className="absolute left-0 top-[0.45em] w-1 h-1 rounded-full bg-slate-700" />
                  {a}
                </li>
              ))}
            </ul>
        ...
        <section className="mb-6">
          ...
          {profile.education.map((edu, i) => (
            <div key={i} className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between text-sm">
              ...
              <div className="text-xs text-slate-500 whitespace-nowrap sm:text-right">{edu.startYear} – {edu.endYear} | GPA: {edu.gpa}</div>
            </div>
          ))}
        </section>
        ...
        {profile.certifications.length > 0 && (
          <section className="mb-6">
            <h2 className="text-base font-bold text-slate-900 uppercase tracking-wider border-b border-slate-200 pb-1 mb-3">Certifications</h2>
            <ul className="space-y-1">
              {profile.certifications.map((cert, i) => (
                <li key={i} className="text-sm text-slate-700">{cert.title} – {cert.institution} ({cert.date})</li>
              ))}
            </ul>
          </section>
        )}
        ...
        {/* Technical Skills */}
        <section>
          <h2 className="text-base font-bold text-slate-900 uppercase tracking-wider border-b border-slate-200 pb-1 mb-3">Technical Skills</h2>
          <div className="space-y-2">
            {skillGroups.map((group) => (
              <div key={group.category} className="text-sm">
                <span className="font-semibold text-slate-900">{group.category}: </span>
                <span className="text-slate-700">{group.skills.map((s) => s.name).join(", ")}</span>
              </div>
            ))}
          </div>
        </section>
